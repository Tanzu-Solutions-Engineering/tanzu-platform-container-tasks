...
schema_file = '/app/z-mount-schema.yml'
values_file = 'z-mount-values.yml'
overlay_file = '/app/z-mount-overlay.yml'
...
configDir = f"{workspaceDir}/package/config/"
...
if "ext.apps.tanzu.vmware.com/mount-secret-as-file" in containerapp["metadata"]["annotations"]:
    ...
    for mount in mountsParsed:
        ...
        volume = {'name': k8s_name,'secret': secretRef['secretKeyRef']['name'],'path': path,'key': secretRef['secretKeyRef']['key'],'varName': name}
